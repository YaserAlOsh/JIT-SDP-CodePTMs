...
class HierachicalRNN(nn.Module):
    ...
    def forward_code(self, x, hid_state):
        ...
        for i in range(n_hunk):
            ...
            for j in range(n_line):
                ...
                sent, state_word = self.wordRNN(torch.LongTensor(words).view(-1, self.batch_size).cuda(), hid_state_word)
                if sents is None:
                    sents = sent
                else:
    ...
    def forward(self, added_code, removed_code, hid_state_hunk, hid_state_sent, hid_state_word):
        ...
        print('subtraction: {} multiple: {} cos: {} euc: {} nn: {} ntn: {}', subtract.shape, multiple.shape, cos.shape,euc.shape, nn.shape, ntn.shape)
