...
def train_model(data, params):
    ...
    params.device = torch.device('cpu' if torch.cuda.is_available() else 'cpu')
    ...
    for epoch in range(1, params.num_epochs + 1):
        ...
        for i, (batch) in enumerate(tqdm(batches)):
            ...
            #pad_added_code = torch.FloatTensor(pad_added_code).to(params.device)
            #pad_removed_code = torch.FloatTensor(pad_removed_code).to(params.device)
