    ...
    for commit in data:
        ...
        for file in commit:
            ...
            for line in file:
                ...
                if len(new_line.split()) == 65:
                     print('pre: {} post: {} max: {}'.format(len(line.split()),len(new_line.split()),max_length))
...
def convert_msg_to_label(pad_msg, dict_msg):
    ...
    for i in range(nrows):
        column = np.unique(pad_msg[i])
        column = column[column < ncols]
...
def mapping_dict_msg(pad_msg, dict_msg):
    return np.array(
        [np.array([dict_msg[w.lower()] if w.lower() in dict_msg.keys() else dict_msg['<NULL>'] for w in line.split(' ')],dtype=object) for line in pad_msg],dtype=object)
    ...
    for commit in pad_code:
        ...
        for file in commit:
            ...
            for line in file:
                new_line = list()
                i=0
                #print(line.translate({ord(c): None for c in '\r'}))#list(filter(lambda s: s not in '\r',line)))
                for token in line.split(' '):
                    ...
                    if token.lower() in dict_code.keys():
                        ...
                        #if token != '<NULL>':
                         #   print('should be null: {} '.format(token))
                        new_line.append(dict_code['<NULL>'])
                if len(new_line) > 64:
                    print('>64 {}\n{}\n{} {} {}'.format(line,new_line,len(line.split(' ')),len(new_line),i))
                new_file.append(np.array(new_line))
                #print('line: {}'.format(len(new_line)))
            #print('file: {}'.format(len(new_file)))
            new_file = np.array(new_file)
            new_files.append(new_file)
        #print('{} {}'.format(len(new_files),new_files))
